...
const HostView = ({ gameId, gameState }) => {
  ...
  if(gameState.status === 'round1_suspect') return <div className="h-full flex flex-col items-center justify-center relative z-20"><h2 className="text-6xl font-bold mb-8 text-red-500 drop-shadow-lg">WHO IS THE KILLER?</h2><div className="text-2xl text-slate-400 mb-8 animate-pulse">Based on first impressions/gut instinct.</div><div className="grid grid-cols-4 gap-4 w-full max-w-5xl">{gameState.players.map(p => <div key={p.uid} className="flex flex-col items-center"><img src={mugshots[p.uid] || 'placeholder'} className="w-32 h-32 rounded-full object-cover border-4 border-slate-700 mb-2"/><div className="text-xl font-bold">{p.name}</div></div>)}</div></div>;

  if(gameState.status === 'round1_weapon') return <div className="h-full flex flex-col items-center justify-center relative z-20"><h2 className="text-6xl font-bold mb-8 text-blue-500 drop-shadow-lg">WHAT DID THEY USE?</h2><div className="text-2xl text-slate-400 mb-8 animate-pulse">Vote for the weapon that fits the crime.</div><div className="flex flex-wrap justify-center gap-4 max-w-6xl">{gameState.possibleWeapons.map(w=><div key={w} className="bg-slate-800 px-6 py-3 rounded-full text-xl border border-slate-600">{w}</div>)}</div></div>;
  ...
  if(gameState.status === 'round2') return (
    <div className="h-full flex flex-col items-center justify-center relative z-20">
        ...
        <div className="text-2xl text-slate-400 mb-8 font-bold">Descriptions provided by 2 INNOCENT witnesses.</div>
        ...
        <p className="text-xl text-slate-400 mb-8">Draw a suspect based on these clues.</p>
        <Timer duration={90} onComplete={setupLineup}/>
    </div>
  );

  if(gameState.status === 'lineup') return <div className="h-full flex flex-col items-center justify-center relative z-20"><h2 className="text-6xl font-bold mb-12">SKETCH VOTING</h2><div className="text-3xl text-slate-400 animate-pulse mb-8">Vote for the drawing that best matches the description.</div><div className="mt-8"><Timer duration={45} onComplete={handleRound2Winner}/></div><button onClick={handleRound2Winner} className="mt-8 bg-slate-800 px-6 py-2 rounded text-slate-400 hover:text-white hover:bg-slate-700">Force End Voting</button></div>;
  ...
  if(gameState.status === 'role_reveal') return <div className="h-full flex flex-col items-center justify-center relative z-20 bg-black"><h1 className="text-8xl font-black text-white mb-8 animate-pulse">CHECK YOUR PHONE</h1><div className="text-3xl text-slate-400 mb-8">Secretly check your device. Don't react.</div><Timer duration={15} onComplete={setupRumors}/></div>;

  if(gameState.status === 'round4_exchange') return <div className="h-full flex flex-col items-center justify-center relative z-20"><h2 className="text-6xl font-bold mb-4">RUMOR MILL</h2><p className="text-3xl text-slate-400 mb-8">Players are verifying (or falsifying) evidence...</p><button onClick={()=>advance('round4_debate')} className="mt-8 bg-slate-700 px-6 py-2 rounded">Force Debate</button></div>;
  ...
  if(gameState.status === 'killing_round') return <div className="h-full flex flex-col items-center justify-center relative z-20"><h2 className="text-6xl font-black mb-8 text-red-600 animate-pulse">SOMEONE IS DYING...</h2><p className="text-2xl text-slate-400">The Killer is choosing a victim.</p></div>;
  ...
  if(gameState.status === 'weapon_clues_murderer' || gameState.status === 'weapon_clues_ghost') return (
    <div className="h-full flex flex-col items-center justify-center relative z-20">
        ...
        <p className="text-2xl text-slate-400 mb-4 animate-pulse">The Killer and Ghost are providing clues...</p>
        <p className="text-lg text-slate-500">The Murderer lies. The Ghost helps.</p>
    </div>
  );
  ...
  if(gameState.status === 'voting') return <div className="h-full flex flex-col items-center justify-center relative z-20"><h2 className="text-7xl font-black mb-8 text-red-500">FINAL JUDGMENT</h2><div className="grid grid-cols-4 gap-4 w-full max-w-6xl mb-8">{gameState.players.map(p=><div key={p.uid} className="flex flex-col items-center"><img src={mugshots[p.uid]} className="w-24 h-24 rounded-full object-cover border-2 border-slate-600 mb-2"/><div className="font-bold">{p.name}</div></div>)}</div><p className="text-2xl text-slate-400">Discuss and cast your final vote. You must get both the Killer and the Weapon correct to win.</p></div>;
...
const PlayerView = ({ gameId, gameState, playerState, user }) => {
  ...
  if(gameState.status === 'round1_suspect') {
    ...
    return (
      <div className="p-4 grid grid-cols-2 gap-4 h-full overflow-y-auto pb-20 relative z-30">
        <div className="col-span-2 text-center text-slate-400 uppercase text-xs font-bold mb-2">Vote for the Killer (Gut Instinct)</div>
  ...
  if(gameState.status === 'round1_weapon') {
    ...
    return (
      <div className="p-4 grid grid-cols-2 gap-4 h-full overflow-y-auto pb-20 relative z-30">
        <div className="col-span-2 text-center text-slate-400 uppercase text-xs font-bold mb-2">Vote for the Weapon (Guess)</div>
    ...
    return (
      <div className="p-4 flex flex-col items-center h-full relative z-30">
        ...
        <div className="bg-slate-800 p-4 rounded mb-4 text-sm w-full">
            <div className="uppercase text-xs text-slate-500 mb-2">Draw the suspect based on the witness statements.</div>
  ...
  if(gameState.status === 'role_reveal') {
      return (
        <div className="h-full flex flex-col items-center justify-center p-6 bg-slate-900 relative z-30">
           ...
           : <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center p-8"><h1 className={`text-5xl font-black mb-4 ${playerState.isMurderer?'text-red-600':'text-blue-500'}`}>{playerState.isMurderer?"MURDERER":"INNOCENT"}</h1><p className="text-slate-400 mb-8">Memorize your role. Keep a poker face.</p><button onClick={()=>setShowRole(false)} className="mt-12 text-slate-500">Hide</button></div>}
        </div>
      );
  }
        ...
        <div className="p-6 h-full flex flex-col relative z-30 overflow-y-auto">
            ...
            {playerState.isMurderer ? (
                <div className="mb-4 p-4 bg-red-900/20 border border-red-500 rounded">
                    <p className="text-red-500 font-bold text-sm uppercase mb-2 flex gap-2"><AlertTriangle className="w-4 h-4"/> Killer: Rewrite to lie.</p>
                ...
                <div className="mb-4">
                     <p className="text-blue-400 font-bold text-xs uppercase mb-2">Innocents: Copy exactly.</p>
  ...
  if(gameState.status === 'killing_round') {
      ...
      return (
        <div className="p-6 h-full flex flex-col justify-center">
            <h2 className="text-3xl font-black text-red-600 mb-6 text-center">Choose your victim wisely.</h2>
            <div className="grid grid-cols-2 gap-4">
      ...
      if(playerState.isMurderer) {
          return (
              <div className="p-6 h-full flex flex-col justify-center">
                  ...
                  <div className="text-sm text-slate-400 mb-4">One word only. Mislead or Help.</div>
  ...
  if(gameState.status === 'voting') {
    ...
    return (
      <div className="p-4 h-full overflow-y-auto pb-32 relative z-30">
        <h2 className="text-xl font-bold mb-4 text-red-500">Select the Killer and the Weapon to win.</h2>
